...
type Props = {
  ...
  onSelect: (enquiry: EnquiryRecord) => void;
};

export const EnquiryPickerList = ({ enquiries, onSelect }: Props) => {
  const { t } = useT();
  ...
  if (enquiries.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-border/70 px-6 py-12 text-center">
        <p className="text-sm font-medium text-foreground">{t("bookings.noOpenEnquiries")}</p>
        <p className="mt-1 text-sm text-muted-foreground">{t("bookings.noOpenEnquiriesHint")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <ListSearchInput value={search} onChange={setSearch} placeholder={t("enquiries.search")} className="sm:max-w-none" />
      {filtered.length === 0 ? (
        <ListSearchEmpty />
      ) : (
        ...
        >
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              {enquiry.eventType} · {format(parseISO(enquiry.preferredDate), "MMM d, yyyy")} · {enquiry.guests}{" "}
              {t("calendar.guests")}
            </p>
            <p className="text-sm text-muted-foreground">{formatEnquiryBudget(enquiry.budget)}</p>
          </div>

...
import { EXTRA_NAV_ITEMS } from "@/config/adminNav";
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
      <div className="rounded-2xl border border-border/70 bg-card p-6 shadow-soft sm:p-8">
        <h2 className="font-display text-xl font-semibold sm:text-2xl">{t("module.extra.title")}</h2>
        <p className="mt-2 text-muted-foreground">{t("module.extra.pageDesc")}</p>
      </div>

      <div>
        <h3 className="mb-4 text-sm font-semibold uppercase tracking-wide text-muted-foreground">
          {t("module.extra.tools")}
        </h3>
        <div className="divide-y divide-border/70 overflow-hidden rounded-2xl border border-border/70 bg-card shadow-soft">
          {EXTRA_NAV_ITEMS.map(({ id, path, icon: Icon, titleKey, descKey }) => (
            <Link
              key={id}
              to={path}
